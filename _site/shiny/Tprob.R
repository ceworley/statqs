...
ui <- fluidPage(
    ...
    radioButtons("unk","What is the unknown?",
                 c("A probability, \\(P\\)" = "P",
                   "A boundary, \\(t^\\star\\)" = "t")),
    ...
    uiOutput("df_statement"),
    conditionalPanel("input.unk == 'P'",
                     numericInput("t","\\(t^\\star\\)=",value=0,step=0.01)),
    conditionalPanel("input.unk == 't' & input.type == 'l'",
                     numericInput("P1","\\(P(T<t^\\star)\\)=",value=0.5,min=0,max=1,step=0.01)),
    conditionalPanel("input.unk == 't' & input.type == 'r'",
                     numericInput("P2","\\(P(T>t^\\star)\\)=",value=0.5,min=0,max=1,step=0.01)),
    conditionalPanel("input.unk == 't' & input.type == 'c'",
                     numericInput("P3","\\(P(|T|<t^\\star)\\)=",value=0.5,min=0,max=1,step=0.01)),
    conditionalPanel("input.unk == 't' & input.type == 'tt'",
...
server <- function(input, output, session) {
    ...
    output$density = renderPlot({
        ...
        if(input$type == "tt"){
            if(t>0){
                ts2 = seq(mint,t1,length.out=100)
                ts3 = seq(t2,maxt,length.out=100)
