...
That spinner ($X$) will be fairly spun `r n` times, and the **total** of the spins will be recorded. Determine the probability that the random total is within `r d` units from $\mu_{\sum X}$.
...
[video](https://vimeo.com/531942365){target="_blank"}
...
plot(Z,dnorm(Z),type="l",xlim=c(-3,3),ylim=c(0,0.4),xlab="",ylab="density",axes=F,lwd=3,
     main=TeX(sprintf("$P(|%s-%s|<%s) = %.4f$","\\sum X",mut,round(d,2),round(ca,4))))
