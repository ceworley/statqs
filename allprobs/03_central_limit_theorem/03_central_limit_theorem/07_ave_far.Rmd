...
That spinner ($X$) will be fairly spun `r n` times, and the **sample mean** of the spins will be recorded. Determine the probability that the random sample mean is farther than $`r d`$ units from $\mu_{\bar{X}}$.
...
[video](https://vimeo.com/531941719){target="_blank"}
...
plot(Z,dnorm(Z),type="l",xlim=c(-3,3),ylim=c(0,0.4),xlab="",ylab="density",axes=F,lwd=3,
     main=TeX(sprintf("$P(|%s-%s|>%s) = %.4f$","\\bar{X}",mua,d,round(ta,4))))
