...
# continuousspinner = function(x=c(0,1),prob=c(1,1),
#                             cex=1,
#                             lwd=1,txd=0.7){
#     theta=seq(0,2*pi,2*pi/1000)
#     plot(sin(theta),cos(theta),type="l",xlim=c(-1.2,1.2),ylim=c(-1.2,1.2))
...
ui <- fluidPage(
    ...
    numericInput("n","n (sample size)",10,1,100,1),
    verbatimTextOutput("sample")
    # tabsetPanel(
    #     tabPanel("Spin", plotOutput("plot")),
    #     tabPanel("Sample", verbatimTextOutput("summary")),
    #     tabPanel("Resample", tableOutput("table"))
    # )
)
...
server <- function(input, output, session) {
    ...
    output$sample = renderPrint({
        if(input$dist=='Bernoulli'){
            rfun = function(n){
                sample(c(0,1),n,TRUE,c(1-input$bernp,input$bernp))
            }
        } else if(input$dist=='dice') {
            rfun = function(n){
                sample(1:input$diceN,n,TRUE)
            }
        } else if(input$dist=='uniform, continuous') {
            rfun = function(n){
                runif(n,input$uca,input$ucb)
            }
        }
        set.seed(input$seed)
        cat(paste(rfun(input$n),collapse=", "))
    })
}
